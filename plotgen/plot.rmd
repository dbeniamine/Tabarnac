...
for(page in unique(acc$addr))
{
    ...
    {
        show(paste("loop current struct", i))
        if(addr < structs[i,]$start)
        {
            #Addr is between two structs
            OK <- T
            show(paste(addr,"between structs n°",i-1,i))
            snames <- c(snames,structs$name[1],structs$name[1])
        }else
        {
            if(addr >= structs[i,]$start && addr < structs[i,]$end)
            {
                # Addr is in the next struct
                show(paste(addr,"in struct", structs[i,]$name))
                OK <- T
                snames <- c(snames, structs$name[i], structs$name[i])
            }else
            {
