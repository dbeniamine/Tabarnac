...
# TODO remove me, usefull for matrix
#acc$T0 <- acc$T1
#acc$T1 <- acc$T2
#acc$T2 <- acc$T3
#acc$T3 <- acc$T4
...
{
    addr=(2^12)*page
    if(addr < structs[2,]$start)
    {
        # Addr is before structs
        snames <- c(snames,structs$name[1])
    }
    else
    {
        # Search for the good structures
        while( !((addr >= structs[i,]$start)  && (addr < structs[i,]$end)))
        {
            #show(paste("addr",addr,"st",structs[i,]$start,"end",structs[i,]$end))
            if(i==1)
                break
            i <- i+1
            if(i > length(structs$name))
            {
                i <- 1 # Unknown struc
                break
            }
        }
        snames <- c(snames, structs$name[i])
    }
}
acc$struct <- snames
#TODO remove me
#acc <- subset(acc, acc$struct!="BT")
#acc <- subset(acc, acc$struct!="D")
#structs <- subset(structs, structs$name!="BT")
#structs <- subset(structs, structs$name!="D")
remove(addr)
remove(base)
remove(page)
remove(snames)
