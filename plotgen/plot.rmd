...
for(str in unique(acc$struct))
{
    ...
    {
        for(j in 2:ncol(sacc))
        {
            ndf <- rbind(c(sacc[i,1], colnames(sacc)[j],sacc[i,j]),ndf)
        }
    }
    ndf <- data.frame(ndf)
    colnames(ndf) <- c("Addr", "Thread", "Density")
    ndf <- transform(ndf, Addr=as.numeric(as.character(Addr)), Density=as.numeric(as.character(Density)))
    # Plot the distribution
    p <- ggplot(ndf,aes(x=Addr, y=Density) ) + geom_line(aes(linetype=Thread, colour=Thread))
    p <- p + geom_point(aes(shape=Thread))
    p <- p + scale_colour_grey()
    p <- p + ggtitle(paste(plotname, str)) + xlab("Address") + ylab("Number of Access")
    show(p)
    ggsave(paste(path, "/",name,"_", str_replace_all(paste(plotname, str), " ", "_"),".eps", sep=""))
}
remove(j)
remove(ndf)
remove(p)
