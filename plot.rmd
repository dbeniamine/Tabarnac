...
{
    lab <- str_replace(x,'-1','Total')
}
my_breaks <- function(x)
{
    breaks <- pretty(x)
    breaks <- c(-1,breaks[breaks>=0 & breaks < nth])
}
...
for(str in unique(acc$struct))
{
    ...
    ndf$Thread <- as.numeric(as.character(str_replace(str_replace(ndf$Thread,'T',''),'total','-1')))
    ndf$Value <- ndf$Value/1000
    mid <- max(subset(ndf,ndf$Thread!=-1)$Value)/2
    ...
    if(bw)
    {
        ...
        p <- p + theme(panel.border = element_blank(),
                       strip.background = element_rect(colour="#CCCCCC"))
        p <- p + scale_colour_gradient2(limits=c(0,2*mid), na.value="black", low="white", mid="grey50",
