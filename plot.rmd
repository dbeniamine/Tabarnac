...
for(s in unique(structsStats$Struct))
{
    ...
    if(ignore && sum(sacc$NbAccess) < maxAcc/minRatio)
    {
...
for(str in unique(acc$struct))
{
    ...
    p <- ggplot(ndf,aes(x=Addr-sacc$addr[1], y=Value/1000) ) + geom_point()
...
for(str in unique(acc$struct))
{
    ...
    #   # Plot the distribution
    p <- ggplot(ndf,aes(x=addr-sacc$addr[1], y=firstacc) ) + geom_point()
    if(titles)
    {
        p <- p + ggtitle(paste(plotname,' "', str,'"',sep=""))
    }
