...
for(str in unique(acc$struct))
{
    ...
    if(bw)
    {
        ...
        p <- p + theme(panel.border = element_blank(),
                       strip.background = element_rect(colour="#CCCCCC"))
        p <- p + scale_colour_gradient2(low="white", mid="light grey",
                high="black", midpoint=mid,name="Number of accesses (x10^3)")
    }
    else
    {
        p <- p + scale_colour_gradient2(low="light yellow", mid="orange",
            high="dark red", midpoint=mid,name="Number of accesses (x10^3)")
    }
    p <- p + theme(legend.position="bottom")
    show(p)
    if(save)
