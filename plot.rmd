...
for(s in unique(structsStats$Struct))
{
    ...
    if(ignore==T && sum(sacc$NbAccess) < maxAcc/minRatio)
    {
...
remove(sacc)
remove(minRatio)
facet_col <- floor(sqrt(length(unique(structsStats$Thread))))
facet_size <- 6*facet_col
pg_size <- 4096
```
...
```{r ByStructsRWPlot, echo=F, fig.width=facet_size, fig.height=facet_size}
...
```{r Distribution, echo=F, fig.width=facet_size, fig.height=facet_size}
