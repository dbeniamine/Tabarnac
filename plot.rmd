...
}else
{
    ...
    p <- p + scale_fill_manual(values=c("#ffeda0", "#f03b20"))
}
...
for(str in unique(acc$struct))
{
    ...
    p <- ggplot(ndf,aes(x=Addr-sacc$addr[1], y=Thread,colour=Value) ) + geom_point() #geom_point(pch=3,cex=1)
    ...
    {
        p <- p + scale_colour_gradient2(low="white", mid="dark orange",
            high="dark red", midpoint=mid,name="Number of\naccesses\n(x10^3)")
    }
    p <- p + theme(legend.position="bottom")
    show(p)
    if(save)
...
for(str in unique(acc$struct))
{
    ...
    #   # Plot the distribution
    p <- ggplot(ndf,aes(x=addr-sacc$addr[1], y=firstacc, colour=firstacc, shape=firstacc) ) + geom_point(pch=3,cex=1)
    p <- p + theme_bw()
    p <- p + ylim(0, nth-1)
    p <- p + theme(legend.position="bottom")
...
#                       panel.grid.major = element_line(colour = "black", size = 0.1))
        p <- p + scale_colour_gradient(limits=c(0,nth-1),name="Thread Id", low="grey70",
                high="black")

    }
    else
    {
        p <- p + scale_colour_gradient(limits=c(0,nth-1),name="Thread Id",
                                      low="#ffeea3", high="#ff0000")
    }
    show(p)
    if(save)
    {
