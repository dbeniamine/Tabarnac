...
if(file.exists(paste(base,".structsModif.csv",sep="")) &&
   file.exists(paste(base,".structsStats.csv",sep="")) &&
    ...
    if(file.exists(paste(base,".remotes.csv",sep=""))){
        use_remote <- T
        acc<-data.frame(read.csv2(file=paste(base,".remotes.csv",sep=""),sep=',',dec='.',stringsAsFactors=FALSE))
    }
}
```
...
if(!InitDone){
    ...
    acc$struct <- snames
    if(use_remote){
        remote <- remote[with(acc,order(addr)),]
        remote$struct <- snames
